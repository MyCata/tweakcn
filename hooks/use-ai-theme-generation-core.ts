...
export function useAIThemeGenerationCore() {
	const generateTheme = useAIThemeGenerationStore(
		(state) => state.generateTheme,
	);
	const loading = useAIThemeGenerationStore((state) => state.loading);
	const cancelThemeGeneration = useAIThemeGenerationStore(
		(state) => state.cancelThemeGeneration,
	);
	const queryClient = useQueryClient();

	const generateThemeCore = async (
